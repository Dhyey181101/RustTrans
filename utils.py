...
def prompt_mistral(prior_prompt,code,counter_examples,output_format):
    if output_format is None:
        output_format="""
        Give me Rust refactoring of above C code.

Here are some constraints contained in <list> tags that you should respect:

<list>

	1. Use the same function name, same argument and return types.
	2. Make sure it includes all imports, uses safe rust, and compiles.
	3. Don't use raw pointers.
	4. Use box pointer whenever possible. Box pointers are preferable to other alternatives.
	5. Try not to use Traits if possible. I would not like to have Traits in resulting Rust code.
	6. Try not to use Generics if possible.
</list>"""
    prompt=[prior_prompt]
    prompt.append({'role':'assistant','context':code})
    new_prompt={"role":"user","context":f""+output_format}
    prompt.append(new_prompt)
    return prompt
def prompt_Llama(prior_prompt,code,counter_examples,output_format):
    if output_format is None:
        output_format="""
        Give me Rust refactoring of above C code.
