...
fn process_lcs_diff(
    ...
    if m > 0 && n > 0 && runestr1[m - 1] == runestr2[n - 1] {
        let mut temp = process_lcs_diff(str1, str2, lcs_matrix.clone(), m - 1, n - 1);
        temp[0].push(' ');
        temp[0].push(runestr1[m - 1]);
        temp[1].push_str("  ");
        diff = temp;
    } else if n > 0 && (m == 0 || lcs_matrix[m][n - 1] > lcs_matrix[m - 1][n]) {
        let mut temp = process_lcs_diff(str1, str2, lcs_matrix.clone(), m, n - 1);
        temp[0].push(' ');
        temp[0].push(runestr2[n - 1]);
        temp[1].push_str(" +");
        diff = temp;
    } else if m > 0 && (n == 0 || lcs_matrix[m][n - 1] <= lcs_matrix[m - 1][n]) {
        let mut temp = process_lcs_diff(str1, str2, lcs_matrix.clone(), m - 1, n);
        temp[0].push(' ');
        temp[0].push(runestr1[m - 1]);
        temp[1].push_str(" -");
        diff = temp;
