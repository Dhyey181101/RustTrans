...
fn local_search(a: &Matrix, b: &Matrix, mut p: Vector, cost: &mut i64) {
    ...
    for _k in 0..2 {
        ...
        for i in 0..nMov - 1 {
            ...
            let j = rng.gen_range(i + 1..nMov);
            swap(&mut move_vec, i as usize, j as usize);
        }
        for i in 0..nMov {
            let r = get(&move_vec, i as usize) / n;
            let s = get(&move_vec, i as usize) % n;
            let d = delta(a, b, &p, r, s);
            if d < 0 {
                *cost += d;
                swap_vector(&mut p, r as usize, s as usize);
                improved = true;
            }
        }
    }
...
fn get(v: &Vec<i64>, i: usize) -> i64 {
    v[i]
}

fn swap_vector(v: &mut Vector, i: usize, j: usize) {
    v.swap(i, j);
}

fn unif(low: i64, high: i64) -> i64 {
    let mut dist = rand::thread_rng();
    low + (dist.gen_range(0..(high - low + 1)) as i64)
}
