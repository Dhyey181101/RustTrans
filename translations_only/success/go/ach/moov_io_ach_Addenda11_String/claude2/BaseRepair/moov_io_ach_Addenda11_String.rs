...
static MOOV_IO_ACH_SPACEZEROS: Lazy<Vec<String>> = Lazy::new(|| generate_padding_strings(94, " "));
static MOOV_IO_ACH_STRINGZEROS: Lazy<Vec<String>> = Lazy::new(|| generate_padding_strings(94, "0"));
...
fn generate_padding_strings(max: usize, pad_char: &str) -> Vec<String> {
    let mut v = Vec::with_capacity(max);
    for i in 0..max {
        v.push(std::iter::repeat(pad_char).take(i).collect::<String>());
    }
    v
}
