...
fn alpha_field(s: &str, max: usize) -> String {
    ...
    if len > max {
        ...
        let pad = MOOV_IO_ACH_SPACE_ZEROS.get(&(max - len)).map_or(Box::new(String::new()), |x| x.clone());
        s.to_string() + &pad.to_string()
    }
}
...
fn numeric_field(n: u32, max: usize) -> String {
    ...
    if len > max {
        ...
        let pad = MOOV_IO_ACH_STRING_ZEROS.get(&(max - len)).map_or(Box::new(String::new()), |x| x.clone());
        pad.to_string() + &s
    }
}
