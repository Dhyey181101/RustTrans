...
fn intersection(i: Box<Interval>, oi: Box<Interval>) -> Box<Interval> {
    if is_empty(&oi) {
    ...
    if fast_contains(&i, oi.lo) {
        ...
        return Box::new(Interval { lo: oi.lo, hi: i.hi });
    }
    if fast_contains(&i, oi.hi) {
        return Box::new(Interval { lo: i.lo, hi: oi.hi });
    }
...
fn empty_interval() -> Box<Interval> {
    Box::new(Interval { lo: PI, hi: -PI })
}
