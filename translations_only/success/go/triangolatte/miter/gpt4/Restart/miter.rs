...
fn miter(points: Vec<Point>, mut width: f64) -> Vec<f64> {
    ...
    for i in 1..points.len() - 1 {
        ...
        let tangent = normalize(&add(&normalize(&sub(&points[i + 1], &points[i])), &normalize(&sub(&points[i], &points[i - 1]))));
...
fn calculate_normals(x: f64, y: f64) -> [Point; 2] {
    [normalize(&Point { x: y, y: -x }), normalize(&Point { x: -y, y: x })]
}
...
fn scale(p: &Point, f: f64) -> Point {
    ...
    Point { x: p.x / norm * f, y: p.y / norm * f }
}

fn sub(p: &Point, r: &Point) -> Point {
    Point { x: p.x - r.x, y: p.y - r.y }
}
