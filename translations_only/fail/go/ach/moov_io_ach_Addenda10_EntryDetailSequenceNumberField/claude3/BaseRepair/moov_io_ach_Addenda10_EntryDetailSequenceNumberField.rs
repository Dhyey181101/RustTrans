...
static mut MOOV_IO_ACH_STRING_ZEROS: Option<Box<HashMap<usize, Box<str>>>> = None;

fn moov_io_ach_populate_map(max: usize, zero: &str) -> Box<HashMap<usize, Box<str>>> {
...
fn moov_io_ach_string_zeros() -> Box<HashMap<usize, Box<str>>> {
    unsafe {
