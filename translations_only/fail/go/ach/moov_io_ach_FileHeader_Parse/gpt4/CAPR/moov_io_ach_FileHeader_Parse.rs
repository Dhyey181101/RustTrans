...
impl MoovIoAchFileHeader {
    fn parse(&mut self, record: String) {
        ...
        self.immediate_destination_name = MoovIoAchConverters::parse_string_field_with_opts(runes[40..63].iter().collect::<String>(), &self.validate_opts);
        self.immediate_origin_name = MoovIoAchConverters::parse_string_field_with_opts(runes[63..86].iter().collect::<String>(), &self.validate_opts);
        self.reference_code = MoovIoAchConverters::parse_string_field_with_opts(runes[86..94].iter().collect::<String>(), &self.validate_opts);
    }
}
...
impl MoovIoAchConverters {
    ...
    fn parse_string_field_with_opts(r: String, opts: &Option<MoovIoAchValidateOpts>) -> String {
        match opts {
