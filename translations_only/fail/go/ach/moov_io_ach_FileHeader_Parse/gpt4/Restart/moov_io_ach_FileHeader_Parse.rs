...
impl MoovIoAchFileHeader {
    fn parse(&mut self, record: String) {
        ...
        self.immediate_destination = MoovIoAchConverters::parse_string_field(
            runes[3..13].iter().collect::<String>(),
        );
        self.immediate_origin = MoovIoAchConverters::parse_string_field(
            runes[13..23].iter().collect::<String>(),
        );
        self.file_creation_date = MoovIoAchValidator::validate_simple_date(
            runes[23..29].iter().collect::<String>(),
        );
        self.file_creation_time = MoovIoAchValidator::validate_simple_time(
            runes[29..33].iter().collect::<String>(),
        );
        ...
        self.immediate_destination_name = MoovIoAchConverters::parse_string_field(
            runes[40..63].iter().collect::<String>(),
        );
        self.immediate_origin_name = MoovIoAchConverters::parse_string_field(
            runes[63..86].iter().collect::<String>(),
        );
        self.reference_code = MoovIoAchConverters::parse_string_field(
            runes[86..94].iter().collect::<String>(),
        );
    }
}
