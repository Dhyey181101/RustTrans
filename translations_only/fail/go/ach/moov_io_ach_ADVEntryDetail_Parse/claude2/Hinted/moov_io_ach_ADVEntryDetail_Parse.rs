...
impl MoovIoAchAdvEntryDetail {
    fn parse(&mut self, record: &str) {
        ...
        self.file_identification = parse_string_field(&runes[48..53].iter().collect::<String>());
        self.ach_operator_data = parse_string_field(&runes[53..54].iter().collect::<String>());
        self.individual_name = runes[54..76].iter().collect();
        self.discretionary_data = runes[76..78].iter().collect();
        self.addenda_record_indicator = parse_num_field(&runes[78..79].iter().collect::<String>()); 
