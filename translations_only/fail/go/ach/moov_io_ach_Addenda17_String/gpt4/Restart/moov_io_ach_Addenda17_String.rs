...
impl MoovIoAchAddenda17 {
    fn new(type_code: String, payment_related_information: String, sequence_number: i32, entry_detail_sequence_number: i32) -> Self {
        Self {
            type_code,
...
fn numeric_field(n: i32, max: usize) -> String {
    let s = n.to_string();
    if s.len() > max {
        s.chars().skip(s.len() - max).collect()
    } else {
        let m = max - s.len();
        if let Some(pad) = MOOV_IO_ACH_STRING_ZEROS.get(&m) {
            format!("{}{}", pad, s)
        } else {
            format!("{}{}", "0".repeat(max - s.len()), s)
        }
    }
}
