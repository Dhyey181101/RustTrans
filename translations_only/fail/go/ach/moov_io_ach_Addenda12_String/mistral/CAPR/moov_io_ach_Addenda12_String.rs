...
impl fmt::Display for MoovIoAchAddenda12 {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if self.type_code.is_empty()
            && self.originator_city_state_province.is_empty()
            && self.originator_country_postal_code.is_empty()
            && self.entry_detail_sequence_number == 0
        {
            return Ok(());
        }
        ...
        buf.push_str(&self.originator_city_state_province);
        buf.push_str(&self.originator_country_postal_code);
        buf.push_str(&"              ".repeat(10));
        buf.push_str(&self.entry_detail_sequence_number.to_string().as_str());
