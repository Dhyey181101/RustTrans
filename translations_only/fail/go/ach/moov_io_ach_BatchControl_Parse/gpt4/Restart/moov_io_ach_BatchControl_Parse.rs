...
const MOOV_IO_ACH_SAVINGS_CREDIT: i32 = 32;
const MOOV_IO_ACH_GL_ZERO_DOLLAR_REMITTANCE_CREDIT: i32 = 44;
...
impl MoovIoAchBatchControl {
    fn parse(&mut self, record: String) {
        ...
        self.company_identification = self.converters.parse_string_field_with_opts(&record[44..54], self.validate_opts.as_ref().map(|b| &**b));
        self.message_authentication_code = self.converters.parse_string_field_with_opts(&record[54..73], self.validate_opts.as_ref().map(|b| &**b));
        self.odfi_identification = self.converters.parse_string_field_with_opts(&record[79..87], self.validate_opts.as_ref().map(|b| &**b));
