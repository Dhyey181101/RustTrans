...
fn is_ear(p: &Element) -> bool {
    ...
    loop {
        if let Some(r_elem) = r {
            ...
            if let Some(next_r) = r_elem.next.as_deref() {
                if is_inside_triangle(&a, &b, &c, r_point) && is_reflex(&r_elem.prev.as_ref().map_or(r_elem.point, |prev| prev.point), r_point, &next_r.point) {
                    return false;
                }
                r = Some(next_r);
            } else {
