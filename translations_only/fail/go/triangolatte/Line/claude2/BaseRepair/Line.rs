...
fn normal(points: Vec<Point>, width: f64) -> Vec<f64> {
    ...
    for i in 0..points.len()-1 {
        ...
        let n1 = normalize_point(Point { x: dy, y: -dx });
        let n2 = normalize_point(Point { x: -dy, y: dx });
        
        let n1 = scale_point(n1, width / 2.0);
        let n2 = scale_point(n2, width / 2.0);
...
fn scale_point(p: Point, f: f64) -> Point {
...
fn miter(points: Vec<Point>, width: f64) -> Vec<f64> {
    ...
    let mut miter2 = scale_point(n2[0], width);
    ...
    for i in 1..points.len()-1 {
        ...
        let tangent = add_points(
            normalize_point(sub_points(points[i+1], points[i])),
            normalize_point(sub_points(points[i], points[i-1]))    
        );
        
        let unit_miter = Point { x: -tangent.y, y: tangent.x };
        
        let miter_length = width / dot_product(unit_miter, n2[0]);
        miter2 = scale_point(unit_miter, miter_length);
    ...
    let last_miter = scale_point(n2[0], width);
...
fn calculate_normals(x: f64, y: f64) -> [Point; 2] {
    [
        normalize_point(Point { x: y, y: -x }),
        normalize_point(Point { x: -y, y: x })
    ]
}

fn normalize_point(p: Point) -> Point {
    scale_point(p, 1.0)
}

fn sub_points(p: Point, r: Point) -> Point {
    Point {
        x: p.x - r.x,
        y: p.y - r.y
    }
...
fn dot_product(p: Point, r: Point) -> f64 {
    p.x * r.x + p.y * r.y   
}
