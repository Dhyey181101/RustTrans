...
fn combine_polygons(outer: &[Point], inner: &[Point]) -> Result<Vec<Point>, Box<dyn Error>> {
    let mut x_max = 0.0;
    let mut m_index = 0;
    ...
    for i in 0..inner.len() {
        result.push(inner[cyclic((m_index as isize + i as isize), inner.len())]);
    }
...
fn find_k(m: Point, outer: &[Point]) -> Result<(Point, usize, usize, bool), Box<dyn Error>> {
    ...
    for (i, &point) in outer.iter().enumerate() {
        let j = if i == outer.len() - 1 { 0 } else { i + 1 };
...
fn are_all_outside(m: Point, k: Point, p_index: usize, outer: &[Point]) -> bool {
    outer.iter().enumerate().all(|(i, &point)| {
        i == p_index || !is_inside_triangle(m, k, outer[p_index], point)
    })
}

fn is_inside_triangle(a: Point, b: Point, c: Point, p: Point) -> bool {
    let condition1 = (c.x - p.x) * (a.y - p.y) - (a.x - p.x) * (c.y - p.y) >= 0.0;
    let condition2 = (a.x - p.x) * (b.y - p.y) - (b.x - p.x) * (a.y - p.y) >= 0.0;
    let condition3 = (b.x - p.x) * (c.y - p.y) - (c.x - p.x) * (b.y - p.y) >= 0.0;

    condition1 && condition2 && condition3
}

fn find_closest(m: Point, k: Point, p_index: usize, outer: &[Point]) -> isize {
    ...
    let mut closest = -1isize;
    let mut max_dist = -1.0;

    for &i in reflex.iter() {
        let dist = outer[i].distance2(m);
        if dist > max_dist {
            closest = i as isize;
            max_dist = dist;
        }
