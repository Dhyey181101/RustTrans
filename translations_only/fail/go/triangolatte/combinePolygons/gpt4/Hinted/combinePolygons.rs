...
fn combine_polygons(outer: Vec<Point>, inner: Vec<Point>) -> Result<Vec<Point>, Box<dyn Error>> {
    ...
    if let Some(e) = err {
        return Err(Box::new(MyError::new(&e)));
    }
...
fn find_k(m: &Point, outer: &Vec<Point>) -> (Point, usize, usize, Option<String>) {
    ...
    for i in 0..outer.len() {
        let j = if i + 1 == outer.len() { 0 } else { i + 1 };
...
fn are_all_outside(m: &Point, k: &Point, p_index: usize, outer: &Vec<Point>) -> bool {
    for (i, point) in outer.iter().enumerate() {
        ...
        if is_inside_triangle(m, k, &outer[p_index], point) {
            return false;
        }
    }
    true
