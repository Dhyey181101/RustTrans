...
fn augment(env: &mut Env) {
    ...
    for i in 0..env.n {
        env.slack[i as usize] = env.lx[r as usize] + env.ly[i as usize] - env.g.get(r, i);
    ...
    'outer: loop {
        while rd < wr {
            ...
            let i = q[rd as usize];
            for j in 0..env.n {
                if env.g.get(i, j) == env.lx[i as usize] + env.ly[j as usize] && !env.t[j as usize] {
...
fn add(env: &mut Env, i: i64, p: i64) {
    ...
    for j in 0..env.n {
        if env.lx[i as usize] + env.ly[j as usize] - env.g.get(i, j) < env.slack[j as usize] {
            env.slack[j as usize] = env.lx[i as usize] + env.ly[j as usize] - env.g.get(i, j);
            env.slackx[j as usize] = i;
        }
    }
}
