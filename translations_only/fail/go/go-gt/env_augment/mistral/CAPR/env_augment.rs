...
impl Env {
    fn augment(&mut self) {
            ...
            for rd in 0..wr {
                ...
                for j in 0..self.n {
                    if self.g.get(i, j) == self.lx[i as usize] + self.ly[j as usize] && !self.t[j as usize] {
                        ...
                        wr += 1;
                        q_index += 1;
                        q[q_index as usize] = self.yx[j as usize];
        ...
        if j < self.n {
            ...
            let mut j_ptr: i64 = j;
            loop {
                let k: i64 = self.xy[i_ptr as usize];
                self.yx[j_ptr as usize] = i_ptr;
                self.xy[i_ptr as usize] = j_ptr;
                i_ptr = self.prev[i_ptr as usize];
                j_ptr = k;
                if i_ptr == -2 {
                    break;
                }
