...
fn qap_solve_ts(a: &Matrix, b: &Matrix, p: &mut Vector, opt: i64, tabu_duration: i64, aspiration: i64, nr_iterations: i64) -> i64 {
    ...
    for iter in 0..nr_iterations {
        ...
        for i in 0..n - 1 {
            for j in i + 1..n {
                let autorized = tabu_list.get(i, p.0[j as usize]) < iter || tabu_list.get(j, p.0[i as usize]) < iter;
                let aspired = tabu_list.get(i, p.0[j as usize]) < iter - aspiration || tabu_list.get(j, p.0[i as usize]) < iter - aspiration || current_cost + dist.get(i, j) < best_cost;

                if (aspired && !already_aspired) || (aspired && already_aspired && dist.get(i, j) < min_dist) || (!aspired && !already_aspired && dist.get(i, j) < min_dist && autorized) {
                    i_retained = i;
                    j_retained = j;
