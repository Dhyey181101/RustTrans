    ...
    while best_cost > opt && iter < nr_iterations  {
        ...
        for i in 0..(n - 1) {
            for j in (i + 1)..n {
                ...
                match (aspired, already_aspired, new_dist.cmp(&min_dist)) {
                    (true, _, Ordering::Less) |
                    (true, true, _) |
                    (false, false, Ordering::Less) if autorized => {
                        i_retained = i;
                        j_retained = j;
                        min_dist = new_dist;
                        if aspired {
        ...
        if i_retained == INF {
            ...
            let z = iter + random_tabu_duration(tabu_duration);
...
fn delta_part(
    ...
    let n = p.len();
    let mut sum = dist.get(i, j);

    for k in 0..n {
        if k != i && k != j && k != r && k != s {
            sum += (a.get(r, i) - a.get(r, j) + a.get(s, j) - a.get(s, i)) * 
...
fn random_tabu_duration(tabu_duration: i64) -> i64 {
    use rand::Rng;
    let mut rng = rand::thread_rng();
    (rng.gen::<f64>().powi(3) * tabu_duration as f64) as i64
}
