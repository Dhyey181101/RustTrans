...
fn gen_trace(p: &mut Vector, trace: &mut Matrix) {
    let n = p.0.len() as i64;
...
fn local_search(a: &Matrix, b: &Matrix, p: &mut Vector, cost: &mut i64) {
    ...
    while improved {
        improved = false;
        for i in 0..move_vec.len() as i64 - 1 {
            let j = unif(i + 1, move_vec.len() as i64 - 1);
            move_vec.swap(i as usize, j as usize);
        }
        for &move_ in &move_vec {
            let r = move_ / n;
            let s = move_ % n;
