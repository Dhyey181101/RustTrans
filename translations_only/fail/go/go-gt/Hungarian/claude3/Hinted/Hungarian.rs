...
fn init_h(g: &Matrix) -> Env {
    ...
    for i in 0..e.n {
        for j in 0..e.n {
            e.lx[i as usize] = max(e.lx[i as usize], g.get(i as usize, j as usize));
        }
    }
    e
}
...
impl Env {
    ...
    fn augment(&mut self) {
        ...
        for i in 0..self.n {
            if self.xy[i as usize] == -1 {
                ...
                for j in 0..self.n {
                    self.slack[j as usize] = self.lx[i as usize] + self.ly[j as usize]
                        - self.g.get(i as usize, j as usize);
        ...
        loop {
            while rd < wr {
                ...
                let i = q[rd as usize];
                for j in 0..self.n {
                    if self.g.get(i as usize, j as usize) == self.lx[i as usize] + self.ly[j as usize]
                        && !self.t[j as usize]
                    {
                        if self.yx[j as usize] == -1 {
                            ...
                            let mut k = j;
                            loop {
                                let mut l = self.xy[k as usize];
                                self.yx[k as usize] = k;
                                self.xy[k as usize] = l;
                                if self.prev[k as usize] == -2 {
                                    break;
                                }
                                k = self.prev[k as usize] as i64;
                            }
                            self.augment();
                            return;
                        }
            ...
            for j in 0..self.n {
                if !self.t[j as usize] && self.slack[j as usize] == 0 {
                    if self.yx[j as usize] == -1 {
                        let mut i = self.slackx[j as usize];
                        self.m += 1;
                        loop {
                            let mut k = self.xy[i as usize];
                            self.yx[k as usize] = i as i64;
                            self.xy[i as usize] = j;
                            if self.prev[i as usize] == -2 {
                                break;
                            }
    ...
    fn add(&mut self, i: i64, p: i64) {
        ...
        for j in 0..self.n {
            if self.lx[i as usize] + self.ly[j as usize] - self.g.get(i as usize, j as usize)
                < self.slack[i as usize]
            {
                self.slack[i as usize] =
                    self.lx[i as usize] + self.ly[j as usize] - self.g.get(i as usize, j as usize);
