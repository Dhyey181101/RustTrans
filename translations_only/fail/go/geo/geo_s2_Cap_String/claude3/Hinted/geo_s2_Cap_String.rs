...
fn geo_s2_cap_string(c: &geo_s2_Cap) -> String {
    format!("[Center={:?}, Radius={:.6}]", c.center.geo_r3_vector, c.radius())
}

fn geo_s2_cap_radius(c: &geo_s2_Cap) -> f64 {
...
impl geo_s2_Cap {
    fn radius(&self) -> f64 {
        geo_s1_angle_degrees(geo_s1_chord_angle_angle(self.radius))
    }
}
