...
fn geo_s1_interval_approx_equal(i: &GeoS1Interval, other: &GeoS1Interval) -> bool {
    ...
    if geo_s1_interval_is_full(i) {
        return geo_s1_interval_length(other) >= 2.0 * (std::f64::consts::PI - GEO_S1_EPSILON);
    }
    if geo_s1_interval_is_full(other) {
        return geo_s1_interval_length(i) >= 2.0 * (std::f64::consts::PI - GEO_S1_EPSILON);
    }
    ...
    (
        (other.lo - i.lo).rem_euclid(2.0 * std::f64::consts::PI).abs() <= GEO_S1_EPSILON
            && (other.hi - i.hi).rem_euclid(2.0 * std::f64::consts::PI).abs() <= GEO_S1_EPSILON
