...
fn geo_s2_cell_id_from_string(s: &str) -> u64 {
    ...
    for i in 2..s.len() {
        let child_pos = (s.as_bytes()[i] - b'0') as usize;
        if child_pos < 0 || child_pos > 3 {
            return 0;
        }
        id = geo_s2_cell_id_children(id)[child_pos];
    }
    id
}
...
fn geo_s2_cell_id_children(ci: u64) -> Box<[u64; 4]> {
    let lsb = geo_s2_cell_id_lsb(ci);
