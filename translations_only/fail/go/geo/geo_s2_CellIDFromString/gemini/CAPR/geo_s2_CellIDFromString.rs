...
pub fn geo_s2_cell_id_from_string(s: &str) -> Option<u64> {
    ...
    let face = s.chars().nth(0).unwrap().to_digit(10).unwrap() as usize;
    if face < 0 || face > 5 || s.chars().nth(1).unwrap() != '/' {
        return None;
    }
    let mut id = geo_s2_cell_id_from_face(face);
    for i in 2..s.len() {
        let child_pos = s.chars().nth(i).unwrap().to_digit(10).unwrap() as usize;
        if child_pos < 0 || child_pos > 3 {
            return None;
        }
        id = children(id)[child_pos];
    }
    Some(id)
}

pub fn geo_s2_cell_id_from_face(face: usize) -> u64 {
    (face as u64) << GEO_S2_POS_BITS | geo_s2_lsb_for_level(0)
}

pub fn geo_s2_lsb_for_level(level: usize) -> u64 {
