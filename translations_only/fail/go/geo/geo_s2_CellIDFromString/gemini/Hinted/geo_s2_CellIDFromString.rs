...
fn geo_s2_cell_id_from_string(s: &str) -> u64 {
    ...
    let face = s.chars().nth(0).unwrap() as u8 - '0' as u8;
    if face < 0 || face > 5 || s.chars().nth(1).unwrap() != '/' {
        return 0;
    }
    let mut id = geo_s2_cell_id_from_face(face);
    for i in 2..s.len() {
        let child_pos = s.chars().nth(i).unwrap() as u8 - '0' as u8;
...
fn geo_s2_cell_id_from_face(face: u8) -> u64 {
    (face as u64) << GEO_S2_POS_BITS | geo_s2_lsb_for_level(0)
}

fn geo_s2_lsb_for_level(level: u8) -> u64 {
    1 << (2 * (GEO_S2_MAX_LEVEL - level) as u64)
}

fn children(ci: u64) -> [u64; 4] {
    let mut ch = [0; 4];
    let lsb = lsb(ci);
    ch[0] = ci - lsb + lsb >> 2;
    let lsb = lsb >> 1;
