...
fn intersects(c: Box<geo_s2_Cap>, other: Box<geo_s2_Cap>) -> bool {
    ...
    match radius_sum.0.partial_cmp(&chord_angle.0) {
...
fn add(c: geo_s1_ChordAngle, other: geo_s1_ChordAngle) -> geo_s1_ChordAngle {
    ...
    let x = (c.0 * (1.0 - 0.25 * other.0)) as f64;
    let y = (other.0 * (1.0 - 0.25 * c.0)) as f64;
...
fn geo_s2_chord_angle_between_points(x: &geo_s2_Point, y: &geo_s2_Point) -> geo_s1_ChordAngle {
    geo_s1_ChordAngle((vector_norm2(&x.vec, &y.vec)).min(4.0))
}
...
fn vector_norm2(v: &geo_r3_Vector, ov: &geo_r3_Vector) -> f64 {
    vector_dot(v, ov)  
}
