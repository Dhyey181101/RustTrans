...
impl GeoS2Cap {
    fn intersects(&self, other: &GeoS2Cap) -> bool {
        ...
        self.radius.0 + other.radius.0 >= geo_s2_chord_angle_between_points(self.center, other.center).0
    }
...
#[derive(Clone, Copy)]
struct GeoS1ChordAngle(pub f64);

impl GeoS1ChordAngle {
    fn add(&self, other: GeoS1ChordAngle) -> Box<GeoS1ChordAngle> {
        if other.0 == 0.0 {
            return Box::new(*self);
        }

        if self.0 + other.0 >= GEO_S1_MAX_LENGTH2 {
            return Box::new(GeoS1ChordAngle(GEO_S1_STRAIGHT_CHORD_ANGLE));
        }

        let x = (self.0 * (1.0 - 0.25 * other.0));
        let y = (other.0 * (1.0 - 0.25 * self.0));
        let result = x + y + 2.0 * (x * y).sqrt();
        Box::new(GeoS1ChordAngle(result.min(GEO_S1_MAX_LENGTH2)))
    }
}
...
fn geo_s2_chord_angle_between_points(x: GeoS2Point, y: GeoS2Point) -> GeoS1ChordAngle {
    ...
    GeoS1ChordAngle(diff.norm2().min(GEO_S1_MAX_LENGTH2))
}
