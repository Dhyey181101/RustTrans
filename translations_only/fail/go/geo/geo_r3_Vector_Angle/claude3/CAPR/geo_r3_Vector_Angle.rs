...
fn geo_r3_vector_angle(v: &GeoR3Vector, ov: &GeoR3Vector) -> GeoS1Angle {
    let cross = geo_r3_vector_cross(Box::new(v.clone()), Box::new(ov.clone()));
    let dot = geo_r3_vector_dot(v, ov);
    GeoS1Angle(dot.atan2(geo_r3_vector_norm(&cross)) * GEO_S1_RADIAN)
}

fn geo_r3_vector_cross(v: Box<GeoR3Vector>, ov: Box<GeoR3Vector>) -> Box<GeoR3Vector> {
    Box::new(GeoR3Vector {
        x: v.y * ov.z - v.z * ov.y,
        y: v.z * ov.x - v.x * ov.z,
        z: v.x * ov.y - v.y * ov.x,
...
fn geo_r3_vector_norm(v: &Box<GeoR3Vector>) -> f64 {
