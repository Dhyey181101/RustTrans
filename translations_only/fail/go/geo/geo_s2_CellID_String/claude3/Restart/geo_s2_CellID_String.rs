...
fn geo_s2_cell_id_string(ci: u64) -> Box<str> {
    if !geo_s2_cell_id_is_valid(ci) {
        return Box::from(format!("Invalid: {:#x}", ci));
    }
...
fn geo_s2_cell_id_level(ci: u64) -> u64 {
    GEO_S2_MAX_LEVEL - geo_s2_find_lsb_set_non_zero_64(ci) / 2
}
...
fn u64_to_string(value: u64) -> Box<str> {
