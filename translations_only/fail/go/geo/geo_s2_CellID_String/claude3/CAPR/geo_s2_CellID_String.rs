...
fn geo_s2_cell_id_is_valid(ci: &Box<geo_s2_CellID>) -> bool {
    geo_s2_cell_id_face(ci) < GEO_S2_NUM_FACES
        && (geo_s2_cell_id_lsb(ci) & 0x1555555555555555 != 0)
}

fn geo_s2_cell_id_face(ci: &Box<geo_s2_CellID>) -> u64 {
    (ci.0 >> GEO_S2_POS_BITS) as u64
}

fn geo_s2_cell_id_lsb(ci: &Box<geo_s2_CellID>) -> u64 {
    ci.0 & (!ci.0).wrapping_add(1)
}

fn geo_s2_cell_id_child_position(ci: &Box<geo_s2_CellID>, level: u64) -> u64 {
    (ci.0 >> (2 * (GEO_S2_MAX_LEVEL - level) + 1)) & 3
}

fn geo_s2_cell_id_level(ci: &Box<geo_s2_CellID>) -> u64 {
