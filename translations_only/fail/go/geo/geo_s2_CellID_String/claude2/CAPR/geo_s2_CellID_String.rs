...
fn lsb(cell_id: &GeoS2CellId) -> u64 {
    cell_id.0 & (!0u64 ^ cell_id.0)  
}

fn child_position(cell_id: &GeoS2CellId, level: i32) -> i32 {
    ((cell_id.0 >> (2*(GEO_S2_MAXLEVEL-level)+1) as u32) & 3) as i32
}
...
fn fmt(cell_id: &GeoS2CellId, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "{}", to_string(cell_id))
}

fn from_str(s: &str) -> Result<GeoS2CellId, ParseIntError> {
