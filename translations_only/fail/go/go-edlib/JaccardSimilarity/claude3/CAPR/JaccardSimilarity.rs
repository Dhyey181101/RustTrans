...
fn jaccard_similarity(str1: &str, str2: &str, split_length: isize) -> f32 {
    ...
    let runestr1: Vec<Box<[char]>> = splitted_str1.iter().map(|s| s.chars().collect::<Vec<_>>().into_boxed_slice()).collect();
    let runestr2: Vec<Box<[char]>> = splitted_str2.iter().map(|s| s.chars().collect::<Vec<_>>().into_boxed_slice()).collect();

    let unionstr = union(&splitted_str1, &splitted_str2);
    let jacc = (runestr1.len() + runestr2.len() - unionstr.len()) as f32;

    jacc / unionstr.len() as f32
}
...
fn union(a: &[String], b: &[String]) -> Vec<Box<[char]>> {
    ...
    for item in b {
        if !m.contains_key(item) {
            m.extend(a.iter().chain(b.iter()).map(|s| (s.clone(), true)));
        }
    }
