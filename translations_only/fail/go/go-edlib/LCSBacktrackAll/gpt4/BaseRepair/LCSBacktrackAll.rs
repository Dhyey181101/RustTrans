...
fn lcs_backtrack_all(str1: &str, str2: &str) -> Result<Vec<String>, Box<dyn Error>> {
    ...
    if rune_str1.is_empty() || rune_str2.is_empty() {
        Err("Can't process and backtrack any LCS with empty string".into())
    } else if rune_str1 == rune_str2 {
        Ok(vec![str1.to_string()])
    } else {
...
fn process_lcs_backtrack_all(
    ...
    if m == 0 || n == 0 {
        ...
        for key in process_lcs_backtrack_all(rune_str1, rune_str2, lcs_matrix, m - 1, n - 1) {
            substrings.insert(format!("{}{}", key, rune_str1[m - 1]));
        }
    } else {
        if lcs_matrix[m - 1][n] >= lcs_matrix[m][n - 1] {
            substrings.extend(process_lcs_backtrack_all(rune_str1, rune_str2, lcs_matrix, m - 1, n));
        }
        if lcs_matrix[m][n - 1] >= lcs_matrix[m - 1][n] {
            substrings.extend(process_lcs_backtrack_all(rune_str1, rune_str2, lcs_matrix, m, n - 1));
        }
    }
