...
fn process_lcs_backtrack_all(
    ...
    if m == 0 || n == 0 {
        ...
        let temp = process_lcs_backtrack_all(str1, str2, lcs_matrix, m - 1, n - 1);
        for s in temp {
            substrings.push(format!("{}{}", s, rune_str1[m - 1]));
        }
    } else {
        if lcs_matrix[m - 1][n] >= lcs_matrix[m][n - 1] {
            let temp = process_lcs_backtrack_all(str1, str2, lcs_matrix, m - 1, n);
            for s in temp {
                substrings.push(s);
            }
        }
        if lcs_matrix[m][n - 1] > lcs_matrix[m - 1][n] {
            let temp = process_lcs_backtrack_all(str1, str2, lcs_matrix, m, n - 1);
            for s in temp {
                substrings.push(s);
            }
        }
