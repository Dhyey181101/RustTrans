...
pub fn strings_similarity(str1: &str, str2: &str, algo: Algorithm) -> f32 {
    match algo {
        ...
        Algorithm::Hamming => {
            let (distance, _) = hamming_distance(str1, str2);
            matching_index(str1, str2, distance)
        }
...
pub fn jaro_similarity(str1: &str, str2: &str) -> f32 {
    ...
    for i in 0..rune_str1_len {
        for j in max(0, i as isize - max_dist as isize)..min(rune_str2_len as isize, i as isize + max_dist as isize + 1) {
            let j = j as usize;
...
pub fn qgram_distance_custom_ngram(
    ...
    for i in splitted_str2.keys() {
        union.entry(i.to_string()).or_insert(0);
    }
