...
fn QgramDistanceCustomNgram(splitted_str1: &HashMap<String, isize>, splitted_str2: &HashMap<String, isize>) -> isize {
    ...
    for (k, v) in splitted_str1 {
        let v2 = splitted_str2.get(k).cloned().unwrap_or(0);
        union.insert(k.clone(), v2);
    }

    for (k, v) in splitted_str2 {
        let v1 = *v;
        let v2 = union.get(k).cloned().unwrap_or(0);
        union.insert(k.clone(), v2);
    }

    let res: isize = union.iter().fold(0, |acc, (k, v)| {
        acc + (splitted_str1[k] as isize - *v as isize).abs()
    });
