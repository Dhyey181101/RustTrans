...
fn QgramDistanceCustomNgram(splitted_str1: &HashMap<String, isize>, splitted_str2: &HashMap<String, isize>) -> isize {
    ...
    for (_, v) in splitted_str1 {
        union.insert("".to_string(), v);
    }
    for (_, v) in splitted_str2 {
        union.insert("".to_string(), v);
    }

    let res: isize = union.iter().fold(0, |acc, (k, v)| acc + (splitted_str1.get(k).unwrap_or(&0) - splitted_str2.get(k).unwrap_or(&0)).abs());
