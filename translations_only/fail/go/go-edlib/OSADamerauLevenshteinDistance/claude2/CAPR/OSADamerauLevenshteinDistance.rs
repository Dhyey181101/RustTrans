...
fn osa_damerau_levenshtein_distance(str1: &str, str2: &str) -> isize {
    ...
    let row = rune_str1len.min(3);
    let mut matrix = vec![vec![0; rune_str2len+1]; row];
    for i in 0..row {
    ...
    for i in 1..=rune_str1len {
        matrix[i as usize % 3][0] = i as isize;
        for j in 1..=rune_str2len {
            ...
            matrix[i as usize % 3][j] = min(min(matrix[(i-1) as usize % 3][j]+1, matrix[i as usize % 3][j-1]+1), matrix[(i-1) as usize % 3][j-1]+count);
            if i > 1 && j > 1 && rune_str1[i-1] == rune_str2[j-2] && rune_str1[i-2] == rune_str2[j-1] {
                matrix[i as usize % 3][j] = min(matrix[i as usize % 3][j], matrix[(i-2) as usize % 3][j-2]+1);
            }
        }
    }
    matrix[rune_str1len as usize % 3][rune_str2len]
}
